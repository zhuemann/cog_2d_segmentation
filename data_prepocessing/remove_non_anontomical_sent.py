...
def remove_non_anatomical_sent_v2(df):
    ...
    df_path = "/UserData/Zach_Analysis/suv_slice_text/uw_all_pet_preprocess_chain_v2/uw_pet_annotated_sentences_v2.xlsx"
    ...
    merged_df = pd.merge(df, df_anotomical_info, on=['Extracted Sentences'])#, left_on='Extracted Sentences', right_on='Extracted Sentences')
    ...
    df_dropped = filtered_df.drop(
        columns=['Petlymph_y', 'Findings_y', 'Impression_y', 'Slice_y', 'SUV_y',
                 'annotation', 'anatomy', 'anatomy_available', 'Unnamed: 0'])

    #df_dropped = filtered_df.drop(columns=['Unnamed: 0', 'annotation', 'anatomy', 'anatomy_available'])
    ...
    print(filtered_df.columns)
    filtered_df.set_index("Petlymph", inplace=True)
    return df_dropped
